...
public class Application {
    public static void main(String... args) {
        ...
        log.info("--- sorting by serial number (ascending) ---");
        airplanes.sort(AirplaneComparator.BySerialNumber);
        airplanes.forEach(airplane -> log.info(airplane.toString()));
        log.info("---");

        log.info("--- sorting by registration id (ascending) ---");
        airplanes.sort(AirplaneComparator.ByRegistrationID);
        airplanes.forEach(airplane -> log.info(airplane.toString()));
        log.info("---");

        log.info("--- sorting by manufacturing date (descending) ---");
        airplanes.sort(AirplaneComparator.ByManufacturingDate);
        airplanes.forEach(airplane -> log.info(airplane.toString()));
        log.info("---");

        log.info("--- sorting by carrier (ascending) ---");
        airplanes.sort(AirplaneComparator.ByCarrier);
        airplanes.forEach(airplane -> log.info(airplane.toString()));
    }
}